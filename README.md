...
到旧排列。页面把时区写作 `Houston, TX`；HTML 和 RSS 仍保留随夏令时变化的
UTC offset。
...
接下来两条检查匿名聚合地图的隐私契约、接口、SVG、全时段聚合和真实 SQLite SQL
...
首次访问会跟随操作系统的明暗偏好；页眉里的 `明` 或 `暗` 表示当前主题。手动切换
...
位置转换为 15° 网格，并在 D1 中保存网格级聚合计数和有限的按日回滚缓冲；不保存
IP、稳定标识、精确位置、访问路径、referrer 或 user agent。公开地图显示自启用以来的
所有成功计入的请求；区域从第 1 次请求开始显示，数量仍只显示区间。地图以朱砂色
羽觞小印标出 15° 粗粒度区域，并用紧凑的五档图例标明累计区间；羽觞由小到大，
朱砂由静至浓，所配一至五道水纹再提供不依赖颜色的第二重编码，不使用连续 color bar。
...
固定的 pixel 和 SVG 地图两条图片路由；没有可打开的文字统计页。清空这两个变量并重新
部署即可恢复零 Worker 请求。404 页面不采集，以免不存在页面和机器人噪声进入统计。

当前正式站已通过这两个仓库变量启用地图；仓库里的 `hugo.yaml` 始终保持关闭，避免
未配置的本地构建或 fork 意外发送请求。后端只使用 Cloudflare Workers Free 和 D1
Free；达到免费配额时允许地图暂时不可用，不应因此开启付费方案。
